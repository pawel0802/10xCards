...
export default function ReviewFlashcards({ initialCandidates }: ReviewFlashcardsProps) {
  ...
  if (loading) return <div className="flex items-center justify-center"><span>Saving...</span></div>;
  if (error) return <div className="text-red-500">{error} <button onClick={handleSubmit}>Retry</button></div>;

  return (
    <div>
      <h2 className="text-xl font-bold mb-4">Review Flashcards</h2>
      <ul className="space-y-4">
        {candidates.map(card => (
          <li key={card.id} className={cn('p-4 border rounded', card.status === 'rejected' && 'opacity-50')}> 
            <input
            ...
            <div className="flex gap-2">
              <button type="button" className={cn('btn', card.status === 'accepted' && 'bg-green-200')} onClick={() => handleAccept(card.id)} disabled={card.status === 'accepted'}>Accept</button>
              <button type="button" className={cn('btn', card.status === 'rejected' && 'bg-red-200')} onClick={() => handleReject(card.id)} disabled={card.status === 'rejected'}>Reject</button>
            </div>
          </li>
        ))}
      </ul>
      <button className="btn mt-6" onClick={handleSubmit} disabled={loading || !hasChanges}>Save Changes</button>
    </div>
  );
}
