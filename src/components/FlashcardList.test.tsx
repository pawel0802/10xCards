...
describe("FlashcardList", () => {
  ...
  it("renders flashcards and shows row actions only when selected", async () => {
    ...
    global.fetch = vi.fn().mockResolvedValueOnce({
      json: () => Promise.resolve({ data: cards, count: 2 }),
    });
    render(<FlashcardList />);
...
it("shows/hides pagination buttons correctly", async () => {
  ...
  global.fetch = vi.fn().mockResolvedValue({ json: () => Promise.resolve({ data: cards.slice(0, 10), count: 25 }) });
  render(<FlashcardList />);
  // Wait for first page
  expect(await screen.findByText("Front 1")).toBeInTheDocument();
  ...
  // Go to page 2
  global.fetch = vi.fn().mockResolvedValue({ json: () => Promise.resolve({ data: cards.slice(10, 20), count: 25 }) });
  ...
  // Go to last page
  global.fetch = vi.fn().mockResolvedValue({ json: () => Promise.resolve({ data: cards.slice(20, 25), count: 25 }) });
  fireEvent.click(screen.getByRole("button", { name: "Next" }));
...
describe("FlashcardList advanced actions", () => {
  ...
  it("opens edit modal and saves changes", async () => {
    global.fetch = vi
      .fn()
      .mockResolvedValueOnce({ json: () => Promise.resolve({ data: [card], count: 1 }) }) // initial fetch
  ...
  it("deletes a flashcard", async () => {
    ...
    global.fetch = vi
      .fn()
      .mockResolvedValueOnce({ json: () => Promise.resolve({ data: cards, count: 2 }) }) // initial fetch
      .mockResolvedValueOnce({ json: () => Promise.resolve({}) }); // DELETE
    render(<FlashcardList />);
    // Wait for the table cell, not the label
  ...
  it("shows toast on error", async () => {
    global.fetch = vi
      .fn()
      .mockResolvedValueOnce({ json: () => Promise.resolve({ data: [card], count: 1 }) }) // initial fetch
      .mockResolvedValueOnce({ json: () => Promise.resolve({ error: "fail" }) }); // DELETE
    render(<FlashcardList />);
    // Wait for the table cell, not the label
    expect((await screen.findAllByText("Front", { selector: "td" })).length).toBeGreaterThan(0);
    window.confirm = vi.fn(() => true);
