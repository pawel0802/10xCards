...
export async function getDueFlashcards(userId: string, limit = 10, requestHeaders?: Headers, cookies?: AstroCookies): Promise<{ data: Flashcard[]; error?: string }>{
  const supabase = createClient(requestHeaders as Headers, cookies as AstroCookies);
...
export async function submitReview(userId: string, flashcardId: string, rating: number, requestHeaders?: Headers, cookies?: AstroCookies, idempotencyKey?: string | null): Promise<{ reviewId?: string; updatedFlashcardFields?: any; deduped?: boolean; error?: string }>{
  const supabase = createClient(requestHeaders as Headers, cookies as AstroCookies);
  ...
  try {
    ...
    return { reviewId: (rpcData as any) ?? null, updatedFlashcardFields };
  } catch (e: any) {
