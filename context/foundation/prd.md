...
- User data privacy: flashcard content and personal data are not exposed to other users or third parties beyond what the AI-powered generation feature requires.
...
- AI flashcard generation completes within 10 seconds for typical text input, with continuous visible feedback to the user during generation.
- The app is usable on the latest two major versions of Chrome, Firefox, Safari, and Edge (mainstream desktop browsers).
- User data — flashcard content and review history — is not accessible to other users or third parties beyond what the AI-powered generation feature requires.

## Business Logic

10xCards makes two domain decisions for the user: it determines which concepts in submitted text are worth learning and how to phrase them as effective flashcard pairs, then determines when each card must be reviewed next based on the learner's recall performance.

**Rule 1 — Concept extraction and card phrasing:** Given a block of raw text, the app determines which concepts are worth learning and phrases them as effective question/answer flashcard pairs. Inputs: user-pasted text. Output: a set of flashcard candidates. The user encounters this rule in the generation review step, where they accept, edit, or discard each candidate.

**Rule 2 — Review scheduling:** Given the user's recall performance on each card (correct / incorrect / difficulty rating per review), the app determines when to surface each card again. Inputs: review outcomes per card. Output: the next scheduled review date per card. The user encounters this rule as the order and timing of cards in a review session.

## Access Control

Login required — email/password or third-party sign-in. Flat user model: every authenticated user has the same access level. There are no admin or guest roles in MVP.
...
- **No custom spaced repetition algorithm.** An existing spaced repetition algorithm will be integrated. Building a proprietary scheduling algorithm from scratch is out of scope — it would consume the entire MVP timeline with no user-visible payoff.
...
- **No integrations with other learning platforms.** No export to third-party flashcard applications, LMS connectors, or platform integrations in MVP.
- **No mobile apps.** Web only for MVP. Native mobile (iOS/Android) is an explicit non-goal at this stage.

## Open Questions

No open questions. All sections are fully populated from the shaping session (`context/foundation/shape-notes.md`, `quality_check_status: accepted`).
